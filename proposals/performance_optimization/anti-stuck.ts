...
export class AntiStuckManager {
  ...
  /**
   * Optimized: Recover monitoring state from Database
   * Ensures that tasks started before a crash are still monitored for stuck state
   */
  public async syncWithDatabase() {
    console.log('[AntiStuck] Syncing monitoring state with DB...');
    const activeTasks = await this.db.task.findMany({
  ...
  /**
   * Optimized: Periodic full scan to catch tasks that missed the event loop
   */
  private setupFullScan() {
    setInterval(() => this.syncWithDatabase(), 60000); // Every minute
  }
