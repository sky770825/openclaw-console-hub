...
- **目前用法**：達爾「需要時查」，但沒有從 AGENTS.md 導航過去的機制
- **問題**：達爾不知道什麼時候該查哪份 cookbook
...
### 實際效益
- 每次對話啟動省 ~700 tokens
- 但更大的價值是**結構化**：達爾知道「去哪裡找」而不是「記住所有東西」
...
| 風險 | 嚴重度 | 緩解 |
|------|--------|------|
| 達爾忘記查 cookbook | 中 | 導航表在 AGENTS.md 裡，每次都看得到 |
| cookbook 路徑找不到 | 低 | 用 semantic_search 也能找到 |
| 模型表不在手冊裡，選錯模型 | 低 | ask_ai 有自動升級鏈，選錯也會被修正 |
| 主人覺得 AGENTS.md 少了東西 | 中 | 只是「移到 cookbook」，不是刪除 |
