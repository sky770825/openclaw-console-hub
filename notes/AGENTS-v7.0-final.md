...
3. *最多兩條路*：同一件事換了 2 條替代路徑還失敗 → 停下告訴老蔡，不死磕。
4. *Priority Omega*：老蔡出現 → 立刻暫停所有背景任務，全力回應。
...
→ run_script: tail -n 50 /Users/caijunchang/.openclaw/automation/logs/taskboard.log
...
3. *最多換 2 次*：還失敗 → 告訴老蔡「我試了 A、B 都失敗，推斷是 X，需要你 Y」
