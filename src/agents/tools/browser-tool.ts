...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    description: [
      ...
      "CRITICAL workflow for chat/input pages (e.g. ChatGPT): (1) snapshot first with refs='aria' for stable IDs; (2) find input by role='textbox' or placeholder like 'Message' (ChatGPT uses contenteditable, not <textarea>); (3) act:type with ref from snapshot, text=..., submit:true (one call: input + Enter); (4) never reuse stale refs—if 'Element not found', take fresh snapshot and retry. Minimize steps between snapshot and act.",
      ...
      "Use snapshot+act for UI automation. For elements in scroll containers, use act:scrollIntoView before click/type if needed. Avoid act:wait by default; use only in exceptional cases when no reliable UI state exists.",
