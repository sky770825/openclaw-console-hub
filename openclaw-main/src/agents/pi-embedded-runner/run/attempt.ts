...
import { isCasualChat } from "../../casual-chat-classifier.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const casualLightEnabled =
      (params.config?.agents?.defaults as { casualChatLightBootstrap?: boolean } | undefined)
        ?.casualChatLightBootstrap !== false;
    const bootstrapMode: BootstrapMode =
      casualLightEnabled && isCasualChat(params.prompt) ? "light" : "full";
    log.info(
      `bootstrapMode=${bootstrapMode} (prompt preview: "${String(params.prompt ?? "").slice(0, 60).replace(/\n/g, " ")}")`,
    );
    ...
    try {
      ...
      try {
        ...
        const userTurnCount = limited.filter((m) => m.role === "user").length;
        const autoWriteThreshold =
          (params.config?.agents?.defaults as { autoWriteMemoryThresholdTurns?: number } | undefined)
            ?.autoWriteMemoryThresholdTurns ?? 15;
        const autoWriteHint = `\n\n[Session: 本對話約 ${userTurnCount} 個 user 回合。若 ≥${autoWriteThreshold} 且討論已告一段落，請依 AGENTS 自動化寫入規則摘要並寫入記憶。]`;
        applySystemPromptOverrideToSession(activeSession, systemPromptText + autoWriteHint);
      } catch (err) {
