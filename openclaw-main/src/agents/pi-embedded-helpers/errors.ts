...
/** Short message for rate limit / API key errors — do not send long error payloads to the user. */
export const RATE_LIMIT_OR_AUTH_USER_MESSAGE =
  "服務暫時無法回覆，請稍後再試。";
...
export function formatRawAssistantErrorForUi(raw?: string): string {
  ...
  if (isRateLimitErrorMessage(trimmed) || isAuthErrorMessage(trimmed)) {
    return RATE_LIMIT_OR_AUTH_USER_MESSAGE;
  }
  ...
  if (httpMatch) {
    ...
    if (!rest.startsWith("{")) {
      const line = `HTTP ${httpMatch[1]}: ${rest}`;
      return line.length > 120 ? RATE_LIMIT_OR_AUTH_USER_MESSAGE : line;
    }
  }
  ...
  if (info?.message) {
    ...
    const full = `${prefix}${type}: ${info.message}${requestId}`;
...
export function formatAssistantErrorText(
  ...
  if (isRateLimitErrorMessage(raw) || isAuthErrorMessage(raw)) {
    return RATE_LIMIT_OR_AUTH_USER_MESSAGE;
  }
