...
│
├── templates/                     # 📋 產品模板
│   ├── real_estate.json           # biz_realestate範本
│   ├── beverage.json              # biz_drinks範本
│   └── window_screen.json         # 防霾biz_window_screen範本
│
├── config/                        # ⚙️ 設定檔
...
│
├── examples/                      # 📊 範例數據
│   ├── sample_data.csv            # biz_realestate範例
│   └── sample_beverage.csv        # biz_drinks範例
│
├── README.md                      # 項目概述
...
```bash
# 建立產品
python src/cli.py create --business biz_drinks
...
- ✅ 住商biz_realestate (biz_realestate特定欄位和模板)
- ✅ biz_drinks (飲品特定欄位和模板)
- ✅ 普特斯防霾biz_window_screen (產品特定欄位和模板)
...
```bash
# 經紀人想快速發布一個房屋到三個平台
python src/cli.py create --business 住商biz_realestate
# 輸入房屋資訊...
python src/cli.py publish --product-id <ID>
...
```bash
# 防霾biz_window_screen公司有 50 個產品要上架
python scripts/batch_upload.py products.csv --business 普特斯防霾biz_window_screen
# ✅ 一次性導入所有產品，可逐一發布
```
...
### 範例代碼
- biz_realestate: `examples/sample_data.csv`
- biz_drinks: `examples/sample_beverage.csv`
