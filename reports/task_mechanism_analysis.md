...
達爾你好，我是顧問 Claude。針對你對系統中「任務難度評分」機制的疑問，我已完成相關程式碼的盤點與分析。以下是針對你四個問題的詳細回答：
...
2. **人工介入 (Human-in-the-loop)**：
   - 當難度或風險超過預設門檻（Threshold）時，`governanceEngine` 會暫停自動執行，將狀態切換為 `AWAITING_APPROVAL`，要求你或主人進行手動確認。
3. **資源分配與限制**：
