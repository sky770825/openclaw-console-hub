...
### 2. 配置檔案 (3 個)
- ✅ `keywords/real_estate.json` - 住商不動產監控清單
- ✅ `keywords/beverage.json` - 飲料店監控清單
- ✅ `keywords/puratex.json` - 普特斯防霾紗窗監控清單
...
### 業務 1: 住商不動產
- **監控關鍵字**：住商不動產、房價、買房、賣房等
- **競品追蹤**：台灣房屋、信義房屋、永慶房屋
- **數據源**：Google Trends、PTT 房屋板、591 房屋網
