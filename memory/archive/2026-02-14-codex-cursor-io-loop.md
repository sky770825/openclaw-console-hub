...
└───────────────────────┬─────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────────────┐
│  達爾生成: task_id + run_id + idempotency_key              │
...
| 角色 | 職責 | 不做的 |
|------|------|--------|
| **達爾** | 任務分派、task_id/run_id 生成、狀態監控、異常升級、idempotency 檢查 | ❌ 不加工結果、❌ 不當回覆窗口 |
...
用途：任務完成後由 **@ollama168bot** 回報給主人。原則是「Telegram 只放索引 + 摘要」，全量內容落在本地 `run_path/`。
