...
「達爾的發想審核中心」已實作完成！這是一個獨立的發想管理系統，審核通過後會自動建立對應的任務到任務板。
...
├── add-dar-idea.sh           # 新增單一發想
├── batch-add-dar-ideas.sh    # 批次新增14個發想
├── approve-idea.sh               # 審核通過並建立任務
└── setup-dar-ideas.sh        # 資料庫設定腳本
...
```bash
./scripts/add-dar-idea.sh "標題" "摘要" "標籤1,標籤2"
```

範例：
```bash
./scripts/add-dar-idea.sh \
  "語音指令系統" \
  "用語音直接指派任務給 Agent" \
  "voice,ui"
```
...
```bash
./scripts/batch-add-dar-ideas.sh
```
...
1. 執行 `./scripts/setup-dar-ideas.sh` 取得 SQL
2. 在 Supabase 執行 SQL 建立表格
3. 執行 `./scripts/batch-add-dar-ideas.sh` 匯入14個發想
4. 在任務板「發想審核」頁面查看並審核！
