...
📁 位置: ~/Desktop/小蔡/backups/2026-02-16-0140/
📝 包含:
• MEMORY.md
• SOUL.md
• BOOTSTRAP.md
...
@caij_n8n_bot
    ↓
xiaocai-maintenance-bot.js (PM2)
    ↓
維護腳本 (scripts/)
    ↓
小蔡系統
...
| 項目 | 位置 |
|------|------|
| Bot 程式 | `~/.openclaw/workspace/skill-github-automation/scripts/xiaocai-maintenance-bot.js` |
| PM2 配置 | `~/.openclaw/workspace/skill-github-automation/ecosystem.config.js` |
| 維護腳本 | `~/.openclaw/workspace/scripts/` |
| 日誌 | `~/.pm2/logs/xiaocai-bot-*.log` |

### PM2 管理

```bash
# 查看狀態
pm2 status xiaocai-bot

# 查看日誌
pm2 logs xiaocai-bot

# 重啟
pm2 restart xiaocai-bot

# 停止
pm2 stop xiaocai-bot
...
**A**:
1. 檢查 PM2 狀態：`pm2 status xiaocai-bot`
2. 查看日誌：`pm2 logs xiaocai-bot --err`
3. 重啟 Bot：`pm2 restart xiaocai-bot`
...
**A**: 不會。大部分指令只是查詢狀態，修復指令會安全地重啟服務，不會影響小蔡的正常運作。

---

### Q: 可以遠端使用嗎？

**A**: 可以！只要手機有網路，隨時隨地都能透過 Telegram 維護小蔡。
