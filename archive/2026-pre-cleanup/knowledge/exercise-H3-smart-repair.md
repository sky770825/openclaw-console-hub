...
> 本題測試達爾對 auto-executor AI Repair 機制的理解與應用能力。
...
如果達爾能做到以下任一項，主人可以額外給 +1 分（超過 100 分不計入，但記入評語）：
...
- `create_task` 建的任務進 draft，需要主人批准才會執行
- 如果主人不在線，可以用 `query_supabase UPDATE` 把任務狀態從 draft 改為 ready（需主人授權）
