...
async function recoverMonitoringFromDB(prisma: any, monitorCallback: (id: string) => void) {
    const activeTasks = await prisma.task.findMany({
        where: { status: 'running' }
    });
    
    console.log(`[Anti-Stuck] Recovering monitoring for ${activeTasks.length} tasks...`);
    activeTasks.forEach((task: any) => {
        monitorCallback(task.id);
    });
}

// Added Periodic Full Scan
function setupFullScan(prisma: any, checkInterval: number = 300000) { // Default 5 mins
    setInterval(async () => {
        const stuckTasks = await prisma.task.findMany({
            where: {
                status: 'running',
                updatedAt: { lt: new Date(Date.now() - 10 * 60 * 1000) } // 10 mins no update
            }
        });
        // Handle stuck tasks found in scan...
    }, checkInterval);
}
