...
async function createTask(name: string, description?: string, owner?: string): Promise<string> {
  try {
    ...
    const r = await fetch(`${TASKBOARD_BASE_URL}/api/openclaw/tasks?allowStub=1`, {
      ...
      body: JSON.stringify({ name: name.slice(0, 100), status: 'pending', priority: 2, owner: validOwner, description }),
    });
...
- 但 server/src/ 和前端 src/ 裡的 TypeScript 源碼 — 不要自己改。你改了會壞。發現 bug 就用 ask_ai 分析清楚（哪個檔案、第幾行、什麼問題），然後建任務讓 auto-executor 執行，或回報給老蔡。
...
建任務：{"action":"create_task","name":"名稱","description":"描述"}  （owner 預設小蔡，指派給老蔡加 "owner":"老蔡"）
