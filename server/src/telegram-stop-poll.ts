...
async function executeNEUXAAction(action: Record<string, string>): Promise<ActionResult> {
  ...
  log.info(`[NEUXA-Action] type=${type} path=${action.path || ''}`);
...
13-編碼品質 — 寫程式前後必看：TypeScript 常見錯誤、安全編碼、自檢清單
...
async function xiaocaiPoll(): Promise<void> {
  ...
  try {
    ...
    for (const update of data.result) {
      ...
      for (let step = 0; step < MAX_CHAIN_STEPS; step++) {
        ...
        for (const jsonStr of actionMatches) {
          try {
            ...
            const maxOutput = action.action === 'read_file' ? 2000 : 800;
            stepResults.push(`${icon} ${label}: ${result.output.slice(0, maxOutput)}`);
            log.info(`[NEUXA-Chain] step=${step} ${action.action} → ok=${result.ok}`);
          } catch {
      ...
      // 組合最終回覆：自然語言 + action 結果摘要
      if (allActionResults.length > 0) {
        const summary = allActionResults.join('\n');
        ...
        for (let selfDrive = 0; selfDrive < 2; selfDrive++) {
          ...
          for (const jsonStr of driveActions) {
            try {
              ...
              const result = await executeNEUXAAction(action);
              const icon = result.ok ? '✅' : '🚫';
              const driveMax = action.action === 'read_file' ? 2000 : 800;
              driveResults.push(`${icon} ${action.action}: ${result.output.slice(0, driveMax)}`);
              log.info(`[NEUXA-SelfDrive] round=${selfDrive} ${action.action} → ok=${result.ok}`);
            } catch { /* skip */ }
