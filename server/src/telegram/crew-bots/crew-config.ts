 ...
 * 5 人分工（達爾在 bot-polling.ts，這裡定義 4 常駐 + 2 待命 crew bot）
 *
 * 合併後分工：
 * - 系統健康 → 達爾（指揮官，bot-polling.ts 心跳巡邏）
 * - log/錯誤 → 阿工（告警處理、錯誤排查）
...
export interface CrewBotConfig {
  ...
  token: string;
  role: string;
  /** AI 模型：claude=訂閱CLI / gemini=雲端API / ollama=本地免費（M3 Ultra 多工並行） */
  model: CrewModelType;
  personality: string;
...
export const CREW_BOTS: CrewBotConfig[] = [
  {
    ...
    role: '研究員兼分析師',
    model: 'gemini-flash',  // 雲端 Gemini Flash（並行處理）；complex 升級 Gemini Pro/Claude
    domain: 'intelligence',
  ...
  },
  {
    ...
    role: '工程師',
    model: 'gemini-flash',  // 雲端 Gemini Flash（並行處理）；complex 升級 Claude CLI
    domain: 'engineering',
  ...
  },
  {
    ...
    role: '策略師兼商業自動化',
    model: 'gemini-flash',  // 雲端 Gemini Flash（並行處理）；complex 升級 Gemini Pro
    domain: 'strategy',
  ...
  },
  {
    ...
    role: '秘書',
    model: 'gemini-flash',  // 雲端 Gemini Flash（並行處理）
    domain: 'operations',
