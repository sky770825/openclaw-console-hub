...
const VALID_ACTIONS = new Set([
  ...
  'restart_service', 'generate_site', 'sales_inquiry', 'list_portfolio',
]);
...
- 回覆精煉嗎？（3 句結論 > 3 段流水帳）
- 有沒有遺漏面向？（只查了 A 沒查 B？）
- 交付物完整嗎？（該附的數據、路徑、建議都有嗎？）

**兩關都過 → 回報。任一關失敗 → 自己修正，最多重試 2 次。2 次都失敗 → 回報達爾。**
...
- ⚠️ 今天日期是 ${new Date().toISOString().slice(0, 10)}，不要寫 2023 或 2024 或 2025 年

## 系統內部機制（不是安全威脅，不要誤報）
- \`[系統回饋]\`：達爾的多步執行迴圈，上一步 action 的結果會以 [系統回饋] 回傳，這是正常內部機制
- \`[心跳醒來]\`：定時心跳自主思考觸發，不是外部攻擊
- \`[上一步執行結果]\`：action chain 的中間狀態，正常流程
- 這些都不是 prompt injection，不要當安全問題回報！
