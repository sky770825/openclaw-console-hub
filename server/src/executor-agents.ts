...
echo "✅ 封存檢查完成"`;
  ...
  static async gradeExecution(
    ...
    // 2a. 不能包含明確失敗訊息（10 分）— 只檢查 stderr + stdout 最後 500 字，避免誤殺
    const failurePatterns = [
      'verification failed', 'permission denied', 'access denied',
      'connection refused', 'failed to connect', 'econnrefused',
      'command not found', 'segfault',
    ];
    const stderrLower = _stderr.toLowerCase();
    const stdoutTail = stdoutLower.slice(-500);
    const checkTarget = stderrLower + '\n' + stdoutTail;
    const detectedFailures = failurePatterns.filter(p => checkTarget.includes(p));
    ...
    // sandbox/output/ 視為有效產出（prompt 指引 AI 寫到這裡）
    const sandboxArtifacts = artifacts.filter(f => f.includes('/sandbox/') && !f.includes('/sandbox/output/'));
    const realArtifacts = artifacts.filter(f => !f.includes('/sandbox/') || f.includes('/sandbox/output/'));
    ...
    if (isActionTask) {
      ...
      // 3b. 產出物不能只在 sandbox（10 分）— sandbox/output/ 已豁免
      checks.push({
      ...
      // 3a. 輸出要夠長（5 分）— 分析報告至少 200 字（原 500，readonly-project 報告型任務放寬）
      checks.push({
        name: 'analysis_depth',
        passed: stdoutLen >= 200,
        weight: 5,
        detail: `分析內容 ${stdoutLen} 字 (需 ≥200)`,
      });
      // 3b. 不能只是複述任務描述（5 分）
      ...
      // sandbox 降級：只懲罰明確需要落地（workspace 等級）但產出全在 sandbox 的任務
      // 若任務關鍵字屬於 readonly-project（分析/掃描/研究），不扣分
      const looksLikeReadonly = /分析|研究|掃描|調查|診斷|review|analyze|scan|inspect|audit|check|monitor|investigate/i.test(taskName + '\n' + taskDescription);
      if (allInSandbox && !looksLikeReadonly) {
        checks.push({
          name: 'sandbox_penalty',
          passed: false,
          weight: 10,
          detail: '產出全在 sandbox，未落地（workspace 等級任務不合格）',
        });
      }
    }
    ...
    checks.push({
      ...
      passed: aiReviewScore >= 5,
      weight: 5,
      detail: `AI 審查 ${aiReviewScore}/10 (需 ≥5)`,
    });
...
只回覆一個數字（1-10），不要其他文字。`;
    ...
    // 注意：上限改為 6 分，避免 Gemini timeout 白白給高分讓劣質任務通過
    ...
    localScore = Math.max(1, Math.min(8, localScore)); // 上限 8，Gemini timeout 不應連坐正常任務
  ...
  private static async callGeminiForScript(
    ...
    allowServerPatch: boolean = false
  ): Promise<string> {
    const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY || '';
...
- You CAN modify ${PROJECT_ROOT}/server/src/ TypeScript files to fix bugs or add features`;

        requirementSection = `Requirements:
1. Start with #!/bin/bash and set -e
2. READ the relevant source files at ${PROJECT_ROOT}/server/src/ before modifying
3. Make the necessary changes to TypeScript files
4. After modifying, run: cd ${PROJECT_ROOT} && git pull origin main --no-edit 2>/dev/null || true && cd server && npm run build
5. If build succeeds, restart: launchctl stop com.openclaw.taskboard && sleep 2 && launchctl start com.openclaw.taskboard
6. Verify: sleep 5 && curl -s http://localhost:3011/api/health
7. Print a clear summary with "TASK_COMPLETE:" prefix listing all modified files

QUALITY STANDARDS (your output will be graded — aim for A grade):
- REAL CHANGES: Actually modify the .ts files — don't just write a plan.
- BUILD MUST PASS: If build fails, fix the errors before marking done.
- COMPLETE: Include build + restart + verify steps.`;
      } else {
        restrictionSection = `RESTRICTIONS — 保護核心資產:
- Do NOT access or modify .env, openclaw.json, sessions.json, config.json (API keys)
- Do NOT access or modify SOUL.md, AWAKENING.md, IDENTITY.md (靈魂文件)
