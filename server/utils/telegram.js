 ...
 * 發送任務超時通知
 */
export async function notifyTaskTimeout(taskName, taskId, runId, timeoutMinutes) {
 ...
 * 發送任務重試通知
 */
export async function notifyTaskRetry(taskName, taskId, runId, currentRetry, maxRetries, reason) {
    const text = `🔄 <b>任務重試</b>\n\n` +
        `<b>任務：</b>${taskName}\n` +
        `<b>任務 ID：</b>${taskId}\n` +
        `<b>執行 ID：</b>${runId}\n` +
 ...
 * 發送模型降級通知
 */
export async function notifyModelFallback(taskName, taskId, runId, from, to) {
 ...
 * 發送任務失敗通知
 */
export async function notifyTaskFailure(taskName, taskId, runId, error, retryCount) {
 ...
 * 發送任務成功通知
 */
export async function notifyTaskSuccess(taskName, taskId, runId, durationMs) {
 ...
 * 發送工作流程開始通知
 */
export async function notifyWorkflowStart(workflowName, taskCount, executionMode) {
 ...
 * 發送工作流程完成通知
 */
export async function notifyWorkflowComplete(workflowName, total, success, failed) {
